...
public class GridNavigationAgent: INavigationAgent
{
    ...
    //@TODO: 建立非泛型graph基类
    private BoardGraph m_CurrentBoard;
    
    public GridNavigationAgent(IGameEntityLogic entityLogic, BoardGraph board)
    {
    ...
    public bool MoveToDestination(float3 destination)
    {
        ...
        bool result = false;
        if (m_CurrentBoard.BFS(GetCurrentPosition(), samplePosition, path))
        {
            m_PathPoints.Clear();
    ...
    public void UpdateNavigation(float elapsedTime, float realElapsedTime)
    {
        ...
        if (m_NavigationState is ENavigationState.Running)
        {
            ...
            EntityView.ViewPosition = GetCurrentPosition() + m_CurrentMovementDirection * m_CurrentSpeed * elapsedTime;
        }
    }
    ...
    public bool IsValidPosition(float3 inPosition)
    {
        return -InValidPosition.x < inPosition.x && inPosition.x < InValidPosition.x
                                               &&-InValidPosition.y < inPosition.y && inPosition.y < InValidPosition.y
                                               &&-InValidPosition.z < inPosition.z && inPosition.z < InValidPosition.z;
    }

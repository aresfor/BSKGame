...
namespace Game.Gameplay
{
    ...
    public interface IPhysicsUtility : IUtility
    {
        ...
        bool SphereCheck(float3 startTrace, float3 direction, float radius, float distance, int traceFlag,
        ...
        bool SingleLineCheck(float3 startTrace, float3 direction, float distance, int traceFlag,
            ref ImpactInfo impactInfo, bool bDrawLine = false, DebugColor color = DebugColor.kColorGreen,
            float duration = 5.0f);

        bool MultiLineCheck(float3 startTrace, float3 direction, float distance, int traceFlag,
        ...
        int OverlapSphereNonAlloc(float3 startPos, float radius, int traceFlag, ref List<ImpactInfo> impactList,
            int maxCount);

        int BoxCastNonAlloc(float3 startPos, float3 halfExtents, float3 direction, quaternion orientation,
            float maxDistance, int traceFlag, ref List<ImpactInfo> impactList, int maxCount,
        ...
        bool SingleLineCheckNoPhysics(int pawnEntityID, float3 startTrace, float3 direction, float distance,
            int obstacleFlag, ref ImpactInfo impactInfo);

        bool SingleLineCheckNoPhysics(int pawnEntityID, float3 startTrace, float3 direction, float distance,
            int obstacleFlag, ref ImpactInfo impactInfo, bool bDrawLine = false,
            DebugColor color = DebugColor.kColorGray, float duration = 5.0f);

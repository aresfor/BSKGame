...
namespace Game.UIFramework
{
    ...
    {
        private List<Action> addList = new (32);
        private List<Action> removeList = new (32);
        
        private readonly List<Action> callbacks = new (32);
        /// <summary>
        ...
        public void AddListener(Action handler) {
            ...
            for (int i = 0; i < addList.Count; i++)
                if (addList[i] == handler)
                    return;
            if (removeList.Contains(handler))
        ...
        public void RemoveListener(Action handler)
        {
            for (int i = 0; i < removeList.Count; i++)
                if (removeList[i] == handler)
                    return;
            if (addList.Contains(handler))
                addList.Remove(handler);
    ...
    {
        private List<Action<T>> addList = new (32);
        private List<Action<T>> removeList = new (32);

        private readonly List<Action<T>> callbacks = new(32);
        ...
        {
            foreach (Action<T> action in removeList)
                callbacks.Remove(action);
            removeList.Clear();

            foreach (Action<T> action in addList)
            {
                if (!callbacks.Contains(action))
                    callbacks.Add(action);
            }
    ...
    {
        private List<Action<T, U>> addList = new (32);
        private List<Action<T, U>> removeList = new (32);

        private readonly List<Action<T, U>> callbacks = new(32);
        ...
        {
            foreach (Action<T, U> action in removeList)
                callbacks.Remove(action);
            removeList.Clear();

            foreach (Action<T, U> action in addList)
            {
                if (!callbacks.Contains(action))
                    callbacks.Add(action);
            }
    ...
    {
        private List<Action<T, U, V>> addList = new (32);
        private List<Action<T, U, V>> removeList = new (32);

        private readonly List<Action<T, U, V>> callbacks = new(32);
        ...
        {
            foreach (Action<T, U, V> action in removeList)
                callbacks.Remove(action);
            removeList.Clear();

            foreach (Action<T, U, V> action in addList)
            {
                if (!callbacks.Contains(action))
                    callbacks.Add(action);
            }
            ...
            for (int i = 0; i < callbacks.Count; i++)
                callbacks[i].Invoke(arg1, arg2, arg3);
        }
    }
}
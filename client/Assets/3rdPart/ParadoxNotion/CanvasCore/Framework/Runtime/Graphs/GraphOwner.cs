...
namespace NodeCanvas.Framework
{
	...
	public abstract class GraphOwner : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        protected void OnEnable() {
            Log.Info($"[Slider_] FlowCreate Enable,firstActivation == FirstActivation.OnEnable {firstActivation == FirstActivation.OnEnable},enableCalled {enableCalled}");
            if ( firstActivation == FirstActivation.OnEnable || enableCalled ) {
                Log.Info($"[Slider_] FlowCreate Enable,!isRunning {isRunning} || isPaused {isPaused} && enableAction == EnableAction.EnableBehaviour {enableAction == EnableAction.EnableBehaviour}");
                if ( ( !isRunning || isPaused ) && enableAction == EnableAction.EnableBehaviour ) {
                    Log.Info($"[Slider_] StartBehaviour");
                    StartBehaviour();
                }
            }

...
namespace NodeCanvas.Editor
{
    ...
    public partial class GraphEditor : EditorWindow
    {
        ...
        static void SetGraphValue()
        {
            string path = $"{Application.dataPath}/Resources/FlowController/ExportFlowJson/{currentGraph.name}.FS";
            string data = System.IO.File.ReadAllText(path);
            if (!string.IsNullOrEmpty(data))
            {
                if (currentGraph.Deserialize(data, null, true) == false)
                {

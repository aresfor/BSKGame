...
namespace Game.Client
{
    public class UnityPhysicsUtility : IPhysicsUtility
    {
        ...
        {
            List<float3> AllVertices = ListPool<float3>.Get();
            ...
            //回收
            AllVertices.Clear();
            ListPool<float3>.Release(AllVertices);
            return realPoint;
        }
        ...
        public int GetLayerMask(int traceFlag)
        {
            ...
            if ((traceFlag & PhysicsLayerDefine.GetFlag(PhysicTraceType.Graph)) != 0)
            {
                layerMask |= (1 << UnityLayersUtil.LayerMask_Graph);
            }
            return layerMask;
        }

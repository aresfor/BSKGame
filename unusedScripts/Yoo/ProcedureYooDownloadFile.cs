...
{
    public class ProcedureYooDownloadFile:ProcedureBase
    {
        ...
        private float CurrentSpeed
        {
            get
            {
                ...
                var sizeDiff = GameEntry.YooResource.Downloader.CurrentDownloadBytes - _lastUpdateDownloadedSize;
                _lastUpdateDownloadedSize = GameEntry.YooResource.Downloader.CurrentDownloadBytes;
        ...
        {
            var downloader = GameEntry.YooResource.Downloader;
            ...
            ChangeState<ProcedureYooDownloadOver>(_procedureOwner);
        }
        ...
        private void OnDownloadErrorCallback(string fileName, string error)
        {
            UILoadTip.ShowMessageBox($"Failed to download file : {fileName}", MessageShowType.TwoButton,
                ...
                , () => { ChangeState<ProcedureYooCreateDownloader>(_procedureOwner); }, UnityEngine.Application.Quit);
        }
        ...
        private void OnDownloadProgressCallback(int totalDownloadCount, int currentDownloadCount, long totalDownloadBytes, long currentDownloadBytes)
        {
            ...
            string descriptionText = Utility.Text.Format("正在更新，已更新{0}，总更新{1}，已更新大小{2}，总更新大小{3}，更新进度{4}，当前网速{5}/s", 
                ...
                GameEntry.YooResource.Downloader.Progress, 
                Utility.File.GetLengthString((int)CurrentSpeed));
            
            TypeEventSystem.Global.Send<float>(GameEntry.YooResource.Downloader.Progress);
